...
public class SaveModelEditorCmd implements Command<Void>, Serializable {
    ...
    public Void execute(CommandContext commandContext) {
        ...
        try {
            // 【1】若有 editor，则以 editor 为准设置 key、name、category、description，否则修改模型的值
            BpmnModel bpmnModel = null;
            byte[] bytes = null;
            if (editor != null && editor.length() > 0) {
                bytes = editor.getBytes(StandardCharsets.UTF_8);
    ...
    private String getMetaInfo(String name, String description) {
